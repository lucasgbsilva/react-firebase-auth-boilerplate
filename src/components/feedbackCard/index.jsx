...
const FeedbackCard = () => {
    ...
    if (validInputs(subject, description)) {
      ...
      try {
        ...
        if (result.data.success) {
          ...
          setMessage("There is an error in submiting feedback.");
        }
      } catch (error) {
  ...
  return (
    <div className="p-4 border rounded-lg shadow-md overflow-hiden max-w-md md:max-w-2x1">
      {message.length === 0 ? (
        <form className="flex flex-col gap-y-4" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              } text-white font-blod py-2 px-4`}
              type="submit"
            >
              Submit Feedback
            </button>
